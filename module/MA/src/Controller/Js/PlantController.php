...
class PlantController extends \Base\Controller\Js\AbstractActionController
{
	/**
	 * @return ViewModel
	 */
    public function indexAction()
    {
		...
		$collection = $em->getRepository("MA\Entity\ProductivePlant")->findByName(
		...
		$payload = $this->prepareHalCollection($paginator, 'process/plant/json');
    ...
    {
		$e    = new \MA\Entity\ProductivePlant;
		$em   = $this->getEntityManager();
		$form = $this->getServiceLocator()
			->get('FormElementManager')
			->get(\MA\Form\ProductivePlantForm::class);
		...
		if ($this->getRequest()->isPost()) {
			...
			if ($form->isValid()) {
				...
				$payload = [
					'payload' => $this->prepareHalEntity($e, "process/plant/detail/json")
				];
			}
			else {
